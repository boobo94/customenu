...
COPY --from=intermediate /customenu/app/package*.json .
COPY --from=intermediate /customenu/app/babel.config.js .
COPY --from=intermediate /customenu/app/vue.config.js .

# run npm install in our local machine
RUN npm install

# copy the generated modules and all other files to the container
COPY --from=intermediate /customenu/app/src/ ./src
